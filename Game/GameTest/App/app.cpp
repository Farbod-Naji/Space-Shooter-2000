...
{	
	...
	void Print(float x, float y, std::string text_s, float r, float g, float b, void* font) {
		char* text = new char[text_s.size() + 1];
		text_s.copy(text, text_s.size() + 1);
		text[text_s.size()] = '\0';

		App::Print(x, y, text, r, g, b, font);
